...
def stop_ec2(z):
    ...
    #response = client.start_instances(InstanceIds=y[0:z])
...
def get_instances():
    ...
    for instance in avail_instances:
        if instance.id == 'i-0498d79642388440a':
            continue
        else:
            y.append(instance.id)
...
def get_error_perinstance(load_data_perinstance,performance_data_perinstance,x):
    ...
    for i in range(0,len(x)):
        ...
        for j in range(0,len(performance_data_perinstance[i])):
            ...
            f=abs(e-performance_data_perinstance[i][j][1])
            z.append((load_data_perinstance[j][0],f))
...
def vmscaling(x,load_data,performance_data_perinstance,error_perinstance):
    ...
    """if(n>len(x)):
        #start_ec2(x,(n-len(x)))
        print ("create",n-len(x))
    else:
        #stop_ec2(x,(len(x)-n))
       print ("terminate",len(x)-n)"""
